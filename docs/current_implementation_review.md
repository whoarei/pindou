...
本文描述的功能对应 `1.3.0+6` 发布候选。它基于 `v1.2.2`，新增点击/连续绘制画笔、快速轨迹补点、20 步撤销/重做，并保存当前编辑工具和画笔色号。
...
手动工程文件与自动快照使用同一版本化 JSON 格式，当前格式版本为 1。文件内嵌原图 Base64、裁剪/生成参数、颜色矩阵、手工替色结果、显示开关、编辑工具、画笔色号和选区。解码会检查格式标识、版本、尺寸、裁剪范围、颜色索引和色块数量；工程内保存的颜色定义用于保持编辑结果，不依赖未来色库资源是否改变。撤销/重做快照不写入工程文件，恢复后从空历史开始。
...
1. **发布版本一致性**：本次目标标签 `v1.3.0` 必须与 `pubspec.yaml` 的 `1.3.0+6` 版本名一致，并保持 Android build number 递增。
...
- [x] 画笔、快速轨迹补点和 20 步撤销/重做发布前提升到 `1.3.0+6`。
...
- 当前 11 项自动化测试全部通过，其中包含连续笔画、撤销/重做及双指防误画回归。
...
人工验证不替代自动化回归；画笔与撤销/重做以 `v1.3.0` 为首次目标发布版本，发布流程必须重新完成 Android、Windows 和 Release 三个 job。
