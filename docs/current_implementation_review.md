...
本文描述的功能对应 `1.2.1+4` 发布候选。它基于 `v1.2.0`，重点改进色号选择页的全色对比、自适应行列布局和多当前色号标注。
...
- 色号图层可以关闭，编辑模式可以选中多个不同原色，在 32 色桌面表格中同时标注，缩到 500 像素宽时自动减少列数并保持全部颜色可见，选择目标色后完成替换。
...
1. **发布版本一致性**：本次目标标签 `v1.2.1` 必须与 `pubspec.yaml` 的 `1.2.1+4` 版本名一致，并保持 Android build number 递增。
...
人工验证不替代自动化回归；自适应全色表与多当前色号标注以 `v1.2.1` 为首次目标发布版本，发布流程必须重新完成 Android、Windows 和 Release 三个 job。
