...
本文描述的功能对应 `1.4.0+7` 发布候选。它基于 `v1.3.0`，新增无需导入图片的空白图案入口，并让无原图作品完整参与画笔编辑、统计、导出、工程保存和自动恢复。
...
- 可选择品牌色号作为画笔，点击修改单格或单指滑动连续绘制；快速划过时自动补齐中间格。
- 最近 20 次替色或完整笔画可撤销和重做；双指操作只缩放/平移，不继续绘制。
...
- 三套色库及当前品牌。
- 可选的原图全部字节、文件名、宽度和高度；空白图案中这些字段为空。
- 归一化裁剪区域和旋转次数。
- 拼豆宽高、比例锁定、最大颜色数、抖动、网格和色号图层开关。
- 当前生成的 `Pattern`。
- 手掌/选择/画笔工具、画笔色号、当前选中的色块索引及撤销/重做可用状态。
- 色库加载、图案生成、JPG 导出状态。
- 最近错误、操作通知和最近导出位置。
...
手动工程文件与自动快照使用同一版本化 JSON 格式，当前格式版本为 1。文件保存裁剪/生成参数、颜色矩阵、手工替色结果、显示开关、编辑工具、画笔色号和选区；图片工程额外内嵌原图 Base64，空白图案的 `source` 为 `null`。解码会检查格式标识、版本、尺寸、裁剪范围、颜色索引和色块数量；工程内保存的颜色定义用于保持编辑结果，不依赖未来色库资源是否改变。撤销/重做快照不写入工程文件，恢复后从空历史开始。
...
图片导入不是创建图案的前置条件。导入卡片同时提供“新建空白图案”按钮，使用参数面板当前的 8–200 宽高和品牌色库创建单色 `Pattern`。底色取品牌色库第一项；当前三套内置色库的第一项均为 White。空白图案没有 `sourceBytes`、文件名或原图尺寸，但从创建完成开始与图片生成结果共用相同的预览、画笔、统计、导出、工程文件和自动保存流程。
...
- 编辑工具栏提供手掌、选择和画笔三种模式。画笔色号从当前品牌全色表中选择，当前色号以颜色块和编号显示。
- 画笔按下后先使用独立覆盖层实时预览轨迹，抬手时一次性更新 `Pattern`，因此一整笔只生成一条撤销记录。
- 相邻指针事件之间使用 Bresenham 格子连线补齐，快速横向、纵向或斜向滑动不会留下事件采样造成的断点。
- 画笔模式单指用于绘制；检测到第二根手指时提交已有轨迹并暂停绘制，直到全部手指抬起，双指仍交给 `InteractiveViewer` 缩放和平移。
- 批量替色和画笔共享最近 20 条 `Pattern` 快照历史；新编辑会清空重做栈，重新生成或导入图片会清空全部历史。
...
- 图片工程内嵌原图，因此不会依赖原图片路径，但文件大小通常略大于原图；空白图案工程不包含原图。
- 当前没有工程压缩、增量保存、最近文件列表或自动快照轮换；撤销历史只保存在内存，不随工程保存。
...
1. **发布版本一致性**：本次目标标签 `v1.4.0` 必须与 `pubspec.yaml` 的 `1.4.0+7` 版本名一致，并保持 Android build number 递增。
...
5. **编辑历史持久化**：当前 20 步历史只在内存中，是否需要随工程文件恢复。
6. **导出内容**：是否需要把数量、品牌和图例一起导出，而不只是网格与格内色号。
7. **测试覆盖**：当前测试已覆盖连续画笔、轨迹补点、双指隔离和撤销/重做，但平台原生对话框和错误路径仍较少。
...
- 当前 14 项自动化测试全部通过，其中包含空白图案创建、原图元数据清除、无原图自动恢复、连续笔画、撤销/重做及双指防误画回归。
...
人工验证不替代自动化回归；无原图空白创作以 `v1.4.0` 为首次目标发布版本，发布流程必须重新完成 Android、Windows 和 Release 三个 job。
