...
本文描述的功能对应 `1.2.0+3` 发布候选。它包含上一版之后的 Android 图库选择、色号图层、色号编辑和工程持久化；这些能力不属于线上 `v1.1.0`。
...
- 色号编辑模式下，点击色块逐个切换选中状态；可以扩展为全部同色，然后从当前品牌完整色盘搜索并替换。
...
- 色号图层可以关闭，编辑模式可以选中多格、打开品牌色盘并完成替色。
...
1. **发布版本一致性**：本次目标标签 `v1.2.0` 必须与 `pubspec.yaml` 的 `1.2.0+3` 版本名一致，并保持 Android build number 递增。
...
- [x] 发布包含图库选择和工程持久化的新版本前提升到 `1.2.0+3`。
...
人工验证不替代自动化回归；Android 图库选择和本次色号编辑/工程持久化改动均以 `v1.2.0` 为首次目标发布版本，发布流程必须重新完成 Android、Windows 和 Release 三个 job。
