...
工具栏的工程菜单支持保存和打开 `.pindou` 文件。工程文件包含生成参数、色号图层开关、生成结果、手工替色结果、编辑工具和画笔色号；图片生成的工程还会内嵌原图和裁剪区域，空白图案工程不需要原图。工程可复制到其他 Android 或 Windows 设备继续编辑。撤销/重做历史仅保存在本次运行的内存中，不写入工程文件。
...
卸载 Android 应用或清除应用数据会删除自动恢复文件；手动保存的 `.pindou` 工程文件不受影响。图片工程内嵌原图，因此文件大小通常会略大于原图；空白图案工程不包含原图数据，文件会小得多。
...
- 推送 `v` 开头的标签，例如 `git tag v1.4.0 && git push origin v1.4.0`。构建完成后会自动创建 GitHub Release，并附加 Android APK 和 Windows x64 便携 ZIP。
- 在 GitHub 的 Actions 页面手动运行 **Build releases**。手动运行不会创建 Release，APK 和 Windows ZIP 可从该次运行的 Artifacts 下载。

以标签 `v1.4.0` 为例，发布文件名为：

- Android：`PindouStudio-v1.4.0-android.apk`
- Windows：`PindouStudio-v1.4.0-windows-x64.zip`
